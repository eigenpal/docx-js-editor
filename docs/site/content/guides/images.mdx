...
| Standalone VML (`w:pict`)                               | Render the bounded read-only subset described below  |
...
solid rectangles/ellipses, straight line segments with arrows, and straight fit-to-box
WordArt (`_x0000_t136`). Group-local coordinates, picture crops, supported text wrapping,
...
Generated previews use bounded static SVG resources. Embedded members still pass the
normal image validation and decode limits; an external or missing member never triggers
a fetch or a misleading partial group preview.
