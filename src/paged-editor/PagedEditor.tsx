...
const PagedEditorComponent = forwardRef<PagedEditorRef, PagedEditorProps>(
  function PagedEditor(props, ref) {
    ...
    // Sticky X for vertical arrow navigation (client X coordinate)
    const stickyXRef = useRef<number | null>(null);
    // Track the visual line index across consecutive vertical moves
    // to avoid boundary ambiguity when a position belongs to two lines
     ...
     */
    const getCaretClientX = useCallback((pmPos: number): number | null => {
      if (!pagesContainerRef.current) return null;
     ...
     * Falls back to checking paragraph ranges when span ranges don't match
     * (e.g., cursor at paragraph boundary position).
     */
    const findLineElementAtPosition = useCallback((pmPos: number): HTMLElement | null => {
      if (!pagesContainerRef.current) return null;
      ...
      for (const line of Array.from(allLines)) {
        ...
        if (pmPos >= pStart && pmPos <= pEnd) {
          // For multi-line paragraphs, find the right line by checking
          // if this is the first line (for positions at/before content start)
          // or if spans on this line are closest to the position
     ...
     *
     * Tracks the visual line index across consecutive vertical moves to avoid
     * boundary ambiguity (when a PM position belongs to end-of-line-A and
     * start-of-line-B simultaneously).
     */
    const handlePMKeyDown = useCallback(
      (view: EditorView, event: KeyboardEvent): boolean => {
        ...
        // Find current line index - use tracked index if available (avoids
        // boundary ambiguity where a position belongs to two adjacent lines)
        ...
        if (targetIndex < 0 || targetIndex >= allLines.length) {
          // At document boundary - clear tracking and let PM handle
          lastVisualLineIndexRef.current = -1;
          return false;
        }
        ...
        return true; // Prevent PM's default ArrowUp/ArrowDown handling
      },
      [getCaretClientX, findLineElementAtPosition, findPositionOnLineAtClientX]
    );
